...
contract CommonWalletLibrary {
    uint public nextWithdrawTime; //下次取款时间
...
// 更新时间
    ...
    function addToReserve() payable recordAction external returns (uint) {
        require(g.isValidGamer(msg.sender)); //是不是成员
        require(msg.value > 3 ether); //账户金额大于3ETH
...
contract TimeDelayedVault {
    ...
    // 为保证取钱时 assert(withdrawObserver.call(bytes4(sha3("observe()")))) 通过，withdrawObserver 不能为空
    // 建议获得授权的取钱者在取钱之前，调用此方法设置观察地址 ob 为 withdrawObserver 赋值
    // ob 可为任意地址
